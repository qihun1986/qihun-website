...
async function show(cpuModel) {
  title.value = cpuModel ? `${cpuModel} - 历史价格` : '历史价格'
  visible.value = true
  loading.value = true
  hasData.value = false
  ...
  await nextTick()
  const history = await loadHistory(cpuModel)
  loading.value = false
  ...
  if (!window.history.state?.modal) {
    history.pushState({ modal: 'priceChart' }, '')
  }
}
...
function renderChart(history) {
  ...
  chartInstance.setOption({
    ...
    xAxis: {
      ...
      axisLabel: { color: chartLabel, rotate: 45, fontSize: 12 }
    },
    yAxis: {
      type: 'value',
      scale: true,
    ...
    series: [
      {
        ...
        data: newPrices,
        smooth: false,
        lineStyle: { color: chartLinePrimary, width: 2 },
      ...
      },
      {
        ...
        data: usedPrices,
        smooth: false,
        lineStyle: { color: chartLineSecondary, width: 2, type: 'dashed' },
...
const onTouchMove = (e) => {
  ...
  if (movedY > 0) {
    e.currentTarget.style.transform = `translateY(${movedY}px)`
  }
}
const onTouchEnd = (e) => {
  if (movedY > 120) close()
...
onUnmounted(() => {
  if (typeof window !== 'undefined') {
    window.__showPriceChart = undefined
  }
...
.modal-overlay.ready .modal-panel {
...
.chart-loading, .chart-empty {
  ...
  height: 220px; color: var(--text-secondary); font-size: 0.9rem;
}
