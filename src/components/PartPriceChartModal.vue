...
async function show(partModel) {
  title.value = partModel ? `${partModel} - 历史价格` : '历史价格'
  visible.value = true
  loading.value = true
  hasData.value = false
  ...
  await nextTick()
  const history = await loadHistory(partModel)
  loading.value = false
  ...
  await nextTick()
  renderChart(history)
  if (!window.history.state?.modal) history.pushState({ modal: 'partPriceChart' }, '')
}
...
function renderChart(history) {
  ...
  const chartText    = C('--chart-text') || '#333'
  const chartBorder  = C('--chart-border') || '#ccc'
  const chartLabel   = C('--chart-label') || '#666'
  const chartLegend  = C('--chart-legend') || '#333'
  const chartSplit   = C('--chart-split') || 'rgba(0,0,0,0.05)'
  const chartTooltipBg    = C('--chart-tooltip-bg') || '#fff'
  const chartTooltipBorder = C('--chart-tooltip-border') || '#ccc'
  const chartLinePrimary   = C('--chart-line-primary') || '#e67e22'
  const chartLineSecondary = C('--chart-line-secondary') || '#2ecc71'

  if (!chartContainer.value) return
  if (chartInstance) chartInstance.dispose()
  chartInstance = echarts.init(chartContainer.value)
  const dates = history.map(item => item.recorded_at.slice(0, 7))  // 只显示年-月 (YYYY-MM)
